...
public class App implements RequestHandler<Document, String> {
    ...
    private String DYNAMODB_TABLE_NAME = "final_table";
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        // temperature와 LED / humid 값이 이전상태와 동일한 경우 테이블에 저장하지 않고 종료
        //워터 센서 추가 이후에
        // 맨끝에 && document.current.state.reported.water.equals(document.previous.state.reported.water) 추가
        if (document.current.state.reported.temperature.equals(document.previous.state.reported.temperature) &&
                document.current.state.reported.Hummidity.equals(document.previous.state.reported.Hummidity)) {
            return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item()
                        ...
                        .withString("Hummidity", document.current.state.reported.Hummidity)
                        .withString("water", document.current.state.reported.water)
                        .withString("RELAY", document.current.state.reported.RELAY)
